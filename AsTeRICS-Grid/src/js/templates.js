...
templates.getGridItem = function (gridElem, metadata) {
    switch (gridElem.type) {
        ...
        case GridElement.ELEMENT_TYPE_EBOOK_READER: {
            return getGridElementEBReader(gridElem, metadata);
        }
        default: {
...
/*---EBook---*/
function getGridElementEBReader(gridElem, metadata) {
    log.warn('ich bin EBOOK 4')
    gridElem = fillDefaultValues(gridElem);
    let stopClicking = gridElem.additionalProps[GridElement.PROP_YT_PREVENT_CLICK];
    let label = i18nService.getTranslation(gridElem.label);
...
<li class="item" data-w="${gridElem.width}" data-h="${gridElem.height}" data-x="${gridElem.posX}" data-y="${
    ...
    <div class="grid-item-content" tabindex="40" aria-label="${i18nService.t('ELEMENT_TYPE_EBOOK_READER')}" id="${
        gridElem.id
    }" data-id="${gridElem.id}" style="${`border: 1px solid ${getBorderColor(metadata)}`}">
        ${
        stopClicking
        ...
        <div class="eb-container" style="position: absolute; top: 0; bottom: 0; left: 0; right: 0;">
            <div id="player" style="outline: 1px solid; outline-offset: -5px; height: 100%; background-color: black; display: flex; align-items: center; justify-content: center;">
                <i class="fab fas fa-book" style="color: whitesmoke"></i>
            </div>
        </div>
    </div>
</li>`;
    ...
    ariaLabel += actions.reduce((total, action) => {
        switch (action.modelName) {
            ...
            case GridActionYoutube.getModelName():
                ...
                total += ', ';
                break;
            case GridActionEbook.getModelName():
                total += i18nService.t(GridActionEbook.getModelName());
                total += ' ' + i18nService.t(action.action);
                total += ', ';
                break;
            default:
