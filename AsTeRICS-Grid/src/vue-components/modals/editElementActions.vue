        ...
        <ul id="actionList">
            ...
            <li v-for="action in gridElement.actions" class="srow">
                ...
                <div v-if="editActionId === action.id" style="margin-top: 1.5em; margin-bottom: 1em">
                  ...
                  <!--Ebook-->
                  <div v-if="action.modelName == 'GridActionEbook'">
                    <div class="srow">
                      <div class="twelve columns">
                        <label for="ebAction" class="four columns normal-text">{{ $t('eBookAction') }}</label>
                        <select id="ebAction" class="eight columns" v-model="action.action">
                          <option v-for="elmAction in ebookActions" :value="elmAction">
                            {{elmAction | translate}}
                          </option>
                        </select>
                      </div>
                    </div>
                    <div class="srow" v-show="action.action === 'EB_OPEN'">
                      <div class="twelve columns">
                        <label for="selectBook" class="four columns normal-text">{{ $t('ebookToOpen') }}</label>
                        <select id="selectBook" class="eight columns" v-model="action.bookId" @change="selectedEbookChanged(action.bookId)">
                          <option value="" selected>{{ $t('automaticLastRead') }}</option>
                          <!--<option v-for="ebook in gridData.webRadios" :value="ebook.radioId">
                            {{webradio.radioName}}
                          </option>-->
                        </select>
                      </div>
